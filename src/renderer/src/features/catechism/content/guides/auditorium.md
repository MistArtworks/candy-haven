# AUDITORIUM

## One file, made visible

![auditorium-01-stage.png](auditorium-01-stage.png)

Open a file and the stage renders it. The stage is the single focal object this
view is allowed, so the transport, the presets and the file's own particulars
are all drawn plain around it.

There is no library here on purpose — the ARCHIVE catalogues your audio. This
department answers one question, which is what a file sounds like.

## Presets

![auditorium-02-presets.png](auditorium-02-presets.png)

Four renderings of the same signal. They are presentations, not analyses — pick
whichever tells you what you are listening for.

- **WAVEFORM** — amplitude over the length of the file.
- **SPECTRUM** — the frequency content, live.
- **SPECTRAL** — the spectrogram, frequency against time.
- **STATIC** — the file drawn whole, no motion.

> The player can be detached into its own window, and pinned above everything
> else, when you want it beside a DAW rather than in front of it.
